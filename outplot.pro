    ...
    CASE newornot OF
       'New Plot':BEGIN
         ...
         IF KEYWORD_SET(psy) THEN BEGIN
          ...
          imstr.oplplot(0) -> SetProperty, SYMBOL=oplsymbol,LINESTYLE=6
         ENDIF ELSE BEGIN
          imstr.oplplot(0) -> SetProperty, SYMBOL='',LINESTYLE=0
         ENDELSE
         
         imstr.oplplot(0) -> SetProperty, DATAX=xx, DATAY=yy,$
          MIN_VALUE=yr(0),MAX_VALUE=yr(1),HISTOGRA=hist,COLOR=clr
         ...
         IF KEYWORD_SET(error) THEN BEGIN
          ...
          imstr.oplplot(1) -> SetProperty, DATAX=xx, DATAY=yy+error,$
              MIN_VALUE=yr(0),MAX_VALUE=yr(1),LINESTYLE=1,COLOR=clr
          imstr.oplplot(2) -> SetProperty, DATAX=xx, DATAY=yy-error,$
              MIN_VALUE=yr(0),MAX_VALUE=yr(1),LINESTYLE=1,COLOR=clr
...
;         ENDIF
         IF KEYWORD_SET(psy) THEN BEGIN
          ...
          imstr.oplplot(imstr.nplots) -> SetProperty, SYMBOL=oplsymbol,LINESTYLE=6
         ENDIF ELSE BEGIN
          imstr.oplplot(imstr.nplots) -> SetProperty, SYMBOL='',LINESTYLE=0
         ENDELSE
         ...
         IF KEYWORD_SET(error) THEN BEGIN
          imstr.oplplot(imstr.nplots) = OBJ_NEW('IDLgrPlot')
          imstr.oplmodel -> Add, imstr.oplplot(imstr.nplots)
          imstr.oplplot(imstr.nplots) -> SetProperty, DATAX=xx, DATAY=yy+error,$
              MIN_VALUE=yr(0),MAX_VALUE=yr(1),LINESTYLE=1,COLOR=clr
          imstr.nplots=imstr.nplots+1
